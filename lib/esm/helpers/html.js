...
export function generateLineageTableRow(entityType, ds, schema, table, attribute) {
  ...
  datasourceTag.set_content(ds.title);
  schemaTag.set_content(schema.name);
  tableTag.set_content(table.name);

  if (entityType === 'attribute' && attribute) {
    attributeTag.set_content(attribute.name);
    descriptionTag.set_content(attribute.description);
    linkTag.appendChild(generateLink(attribute.url, linkTag));
  } else {
    descriptionTag.set_content(table.description);
