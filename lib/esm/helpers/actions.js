...
export async function uploadPhysic(connector, customFieldsId, physicData) {
  ...
  if (segments.length === 4) {
    ...
    if (physicData.title.length || physicData.description.length) {
      const response = await connector.Attribute.update({
        ...
        ...(physicData.description.length ? {
          description: generateTextElement(physicData.description).toString()
        } : {})
      });
    ...
    lineageTableRow = generateLineageTableRow('attribute', instance, schema, table, attribute); // добавление ссылки Lineage ref
    ...
    lineageTableRow = generateLineageTableRow('table', instance, schema, table); // добавление ссылки Lineage ref
