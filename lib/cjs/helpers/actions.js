...
async function uploadPhysic(connector, customFieldsId, physicData) {
  ...
  const table = (0, _index.checkSearchResult)(await connector.Table.search({
  ...
  if (segments.length === 4) {
    // получение аттрибута
    const attribute = (0, _index.checkSearchResult)(await connector.Attribute.search({
      name: segments[3].toLowerCase(),
      ds_id: instance.id,
      schema_id: schema.id,
      table_id: table.id
    }, true));
    lineageTableRow = (0, _html.generateLineageTableRow)('attribute', instance, schema, table, attribute); // обновление title и description
    ...
    lineageTableRow = (0, _html.generateLineageTableRow)('table', instance, schema, table); // обновление title и description
